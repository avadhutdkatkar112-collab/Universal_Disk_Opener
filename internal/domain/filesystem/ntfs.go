...
func (r *NTFSReader) GetFileContent(diskReader DiskReader, _ uint64, file *FileEntry) ([]byte, error) {
	...
	for i := int64(0); i < totalClusters; i++ {
		clusterOffset := r.partitionStart*r.sectorSize + (uint64(file.ClusterStart)+uint64(i))*uint64(r.clusterSize)
		clusterSectors := uint32(r.clusterSize) / uint32(r.sectorSize)

		data, err := diskReader.ReadSectors(clusterOffset/r.sectorSize, clusterSectors)
		if err != nil {
			break
		}
