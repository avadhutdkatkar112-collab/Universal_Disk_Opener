...
// It bridges the disk.VirtualDisk interface with the filesystem.Reader interface.
package vfs
...
// DiskVFS implements VirtualFS using a disk.VirtualDisk and filesystem.Reader.
type DiskVFS struct {
...
func NewDiskVFS(vdisk disk.VirtualDisk, partitionStart uint64) (*DiskVFS, error) {
	adapter := filesystem.NewDiskAdapter(vdisk)
	fsType := filesystem.DetectFSType(adapter, partitionStart)

	reader, err := filesystem.NewReader(adapter, partitionStart)
	if err != nil {
...
func (v *DiskVFS) ListDirectory(rawPath string) ([]Entry, error) {
	adapter := filesystem.NewDiskAdapter(v.vdisk)
...
func (v *DiskVFS) ReadFile(rawPath string) (io.ReadCloser, error) {
	...
	currentPath := "/"
	var targetEntry *filesystem.FileEntry
