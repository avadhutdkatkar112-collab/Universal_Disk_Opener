...
// It implements the storage.VirtualDisk interface for VHD files.
package vhd
...
func init() {
	storage.Register(".vhd", func() storage.VirtualDisk {
		return &VHDDriver{}
	})
}

// VHDDriver implements storage.VirtualDisk for Microsoft VHD files.
...
func (d *VHDDriver) Info() storage.DiskInfo {
	if d.footer == nil {
		return storage.DiskInfo{}
	}

	info := storage.DiskInfo{
		FilePath:    d.filePath,
		FileName:    d.fileName,
		FileSize:    d.fileSize,
