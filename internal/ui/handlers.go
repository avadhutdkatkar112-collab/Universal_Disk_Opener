...
import (
	...
	hashcap "github.com/user/vhd-opener/internal/capabilities/hash"
	"github.com/user/vhd-opener/internal/artifacts/evtx"
	"github.com/user/vhd-opener/internal/artifacts/mft"
	"github.com/user/vhd-opener/internal/artifacts/registry"
	"github.com/user/vhd-opener/internal/domain/disk"
	"github.com/user/vhd-opener/internal/domain/vfs"
	"github.com/user/vhd-opener/internal/engine/die"
...
type App struct {
	...
	grammar       *grammar.GrammarService
	die           *die.Registry
	gateway       *platform.Gateway
	events        *platform.EventBus
	jobs          *platform.JobManager
...
func NewApp() *App {
	...
	jobManager := platform.NewJobManager(events)
	gateway := platform.NewGateway(events, jobManager)
	p := services.NewPlatform()

	// Register streaming hash capability
	hashCap := hashcap.NewHashingCapability(p.ReadFile)
...
func (a *App) Startup(ctx context.Context) {
	...
	a.grammar = grammar.NewGrammarService(appDir)
...
func (a *App) GetGateway() *platform.Gateway {
	return a.gateway
}
...
func (a *App) GetPartitions() []disk.Partition {
...
func (a *App) ValidateFile(path string) (*disk.ValidationResult, error) {
	return disk.Validate(path)
}
...
func (a *App) GetGrammarForExtension(ext string) (*grammar.GrammarResponse, error) {
...
func (a *App) initDIE() {
	a.die = die.NewRegistry()

	deps := &die.HandlerDeps{
	...
	die.RegisterDefaultHandlers(a.die, deps)
}
...
func (a *App) GetSuggestions(query string, contextJSON string) ([]die.Suggestion, error) {
	if a.die == nil {
		a.initDIE()
	}
...
func (a *App) ParseCommand(command string, contextJSON string) (*die.Intent, error) {
	if a.die == nil {
		a.initDIE()
	}
...
func (a *App) GetCommandHistory() ([]die.HistoryEntry, error) {
	if a.die == nil {
		a.initDIE()
	}
...
func (a *App) GetFavorites() ([]die.FavoriteEntry, error) {
	if a.die == nil {
		a.initDIE()
	}
