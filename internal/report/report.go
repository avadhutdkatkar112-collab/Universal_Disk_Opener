...
type ReportData struct {
	...
	SuspiciousProcesses []memory.ProcessInfo
	YaraMatches         []memory.YaraMatch
	TimelineSample      []timeline.TimelineEntry
}

func CompileReport(caseName, investigator string, entries []timeline.TimelineEntry, alerts []sigma.Alert, procs []memory.ProcessInfo, yaraMatches []memory.YaraMatch) (string, error) {
	suspiciousProcs := make([]memory.ProcessInfo, 0)
	for _, p := range procs {
		if p.IsSuspicious {
