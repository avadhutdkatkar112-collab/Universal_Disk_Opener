...
export async function OpenFile(path: string): Promise<any> {
  const result = await (window as any).go.api.App.OpenFile(path);
  return typeof result === 'string' ? JSON.parse(result) : result;
}

export async function OpenFileDialog(): Promise<string> {
  return await (window as any).go.api.App.OpenFileDialog();
}

export async function OpenFileNative(vfsPath: string, fileName: string): Promise<void> {
  await (window as any).go.api.App.OpenFileNative(vfsPath, fileName);
}

export async function Close(): Promise<void> {
  await (window as any).go.api.App.Close();
}

export async function SelectPartition(index: number): Promise<any> {
  await (window as any).go.api.App.SelectPartition(index);
}

export async function GetRecentFiles(): Promise<any[]> {
  try {
    const result = await (window as any).go.api.App.GetRecentFiles();
    return typeof result === 'string' ? JSON.parse(result) : result || [];
  } catch {
    return [];
  }
...
export async function GetDiskInfo(): Promise<any> {
  const result = await (window as any).go.api.App.GetDiskInfo();
  return typeof result === 'string' ? JSON.parse(result) : result;
}

export async function GetDetailedDiskInfo(): Promise<any> {
  const result = await (window as any).go.api.App.GetDetailedDiskInfo();
  return typeof result === 'string' ? JSON.parse(result) : result;
}

export async function GetDiskHash(): Promise<{ md5: string; sha256: string }> {
  const result = await (window as any).go.api.App.GetDiskHash();
  return typeof result === 'string' ? JSON.parse(result) : result;
}

export async function GetPartitions(): Promise<any[]> {
  const result = await (window as any).go.api.App.GetPartitions();
  return typeof result === 'string' ? JSON.parse(result) : result || [];
}
...
export async function ListDirectory(path: string): Promise<any> {
  const result = await (window as any).go.api.App.ListDirectory(path);
  return typeof result === 'string' ? JSON.parse(result) : result;
}

export async function GetEntry(path: string): Promise<any> {
  const result = await (window as any).go.api.App.GetEntry(path);
  return typeof result === 'string' ? JSON.parse(result) : result;
}
...
export async function ReadFile(path: string): Promise<Uint8Array> {
  const raw: any = await (window as any).go.api.App.ReadFile(path);
...
export async function ReadFileChunk(path: string, offset: number, length: number): Promise<Uint8Array> {
  const raw: any = await (window as any).go.api.StorageHandler.ReadFileChunk(path, offset, length);
  return decodeWailsBytes(raw);
}

export async function ReadFileText(path: string): Promise<string> {
  const raw: any = await (window as any).go.api.StorageHandler.ReadFile(path);
...
export async function ExtractFile(diskPath: string, localPath: string): Promise<void> {
  await (window as any).go.api.App.ExtractFile(diskPath, localPath);
}

// ── Preview & File Info ──────────────────────────────────────────────────

export async function GetFilePreview(path: string): Promise<PreviewResponse> {
  const result = await (window as any).go.api.App.GetFilePreview(path);
  return typeof result === 'string' ? JSON.parse(result) : result;
}

export async function GetGrammarForExtension(ext: string): Promise<GrammarResponse> {
  const result = await (window as any).go.api.App.GetGrammarForExtension(ext);
  return typeof result === 'string' ? JSON.parse(result) : result;
}

export async function GetFileInfoForPath(path: string): Promise<any> {
  const result = await (window as any).go.api.App.GetFileInfoForPath(path);
  return typeof result === 'string' ? JSON.parse(result) : result;
}
...
export async function BrowseLocalFS(dirPath: string): Promise<any[]> {
  const result = await (window as any).go.api.App.BrowseLocalFS(dirPath);
  return typeof result === 'string' ? JSON.parse(result) : result || [];
}

export async function GetLocalDrives(): Promise<string[]> {
  const result = await (window as any).go.api.App.GetLocalDrives();
  return typeof result === 'string' ? JSON.parse(result) : result || [];
}
...
export async function ExecuteCommand(
  ...
  try {
    ...
    const result = await (window as any).go.api.App.ExecuteCommand(command, ctxStr);
    return typeof result === 'string' ? JSON.parse(result) : result;
  } catch (err) {
    return {
      action: 'error',
...
export async function GetSuggestions(
  ...
  try {
    const ctxStr = typeof context === 'string' ? context : JSON.stringify(context);
    const result = await (window as any).go.api.App.GetSuggestions(query, ctxStr);
    return typeof result === 'string' ? JSON.parse(result) : result || [];
  } catch {
    return [];
  }
...
export async function ParseCommand(
  ...
  try {
    const ctxStr = typeof context === 'string' ? context : JSON.stringify(context);
    const result = await (window as any).go.api.App.ParseCommand(command, ctxStr);
    return typeof result === 'string' ? JSON.parse(result) : result;
  } catch {
    return null;
  }
...
export async function GetRegistryArtifacts(
  ...
  const result = await (window as any).go.api.App.GetRegistryArtifacts(hivePath, hiveType);
  return typeof result === 'string' ? JSON.parse(result) : result;
}

export async function ParseEVTXFile(evtxPath: string): Promise<any> {
  const result = await (window as any).go.api.App.ParseEVTXFile(evtxPath);
  return typeof result === 'string' ? JSON.parse(result) : result;
}

export async function ParseMFTFile(mftPath: string): Promise<any> {
  const result = await (window as any).go.api.App.ParseMFTFile(mftPath);
  return typeof result === 'string' ? JSON.parse(result) : result;
}
...
export async function BuildUnifiedTimeline(
  ...
  const result = await (window as any).go.api.App.BuildUnifiedTimeline(
...
export async function ExportTimelineCSV(entries: any[], outputPath: string): Promise<void> {
  await (window as any).go.api.App.ExportTimelineCSV(entries, outputPath);
}

export async function ExportTimelineJSON(entries: any[], outputPath: string): Promise<void> {
  await (window as any).go.api.App.ExportTimelineJSON(entries, outputPath);
}

export async function AnalyzeFindings(entries: any[]): Promise<any[]> {
  const result = await (window as any).go.api.App.AnalyzeFindings(entries);
  return typeof result === 'string' ? JSON.parse(result) : result || [];
}

export async function LoadTimelineToSQL(entries: any[]): Promise<number> {
  return await (window as any).go.api.App.LoadTimelineToSQL(entries);
}

export async function ExecuteSQLQuery(query: string): Promise<any> {
  const result = await (window as any).go.api.App.ExecuteSQLQuery(query);
  return typeof result === 'string' ? JSON.parse(result) : result;
}

export async function LoadSigmaRuleDirectory(dir: string): Promise<number> {
  return await (window as any).go.api.App.LoadSigmaRuleDirectory(dir);
}

export async function RunSigmaScan(entries: any[]): Promise<any[]> {
  const result = await (window as any).go.api.App.RunSigmaScan(entries);
  return typeof result === 'string' ? JSON.parse(result) : result || [];
}

export async function GetSigmaRuleCount(): Promise<number> {
  return await (window as any).go.api.App.GetSigmaRuleCount();
}

export async function GetLiveProcesses(): Promise<any[]> {
  const result = await (window as any).go.api.App.GetLiveProcesses();
  return typeof result === 'string' ? JSON.parse(result) : result || [];
}

export async function RunMemoryYaraScan(procs: any[]): Promise<any[]> {
  const result = await (window as any).go.api.App.RunMemoryYaraScan(procs);
  return typeof result === 'string' ? JSON.parse(result) : result || [];
}

export async function GetMemorySnapshot(): Promise<any> {
  const result = await (window as any).go.api.App.GetMemorySnapshot();
  return typeof result === 'string' ? JSON.parse(result) : result;
}
...
export async function GenerateReport(caseName: string, investigator: string, entries: any[], alerts: any[], procs: any[], yaraMatches: any[]): Promise<string> {
  return await (window as any).go.api.App.GenerateReport(caseName, investigator, entries, alerts, procs, yaraMatches);
}

export async function SaveReportToFile(filePath: string, htmlContent: string): Promise<void> {
  await (window as any).go.api.App.SaveReportToFile(filePath, htmlContent);
}

export async function IngestEvidence(srcPath: string, outputDir: string, passphrase: string, vaultID: string): Promise<any> {
  const result = await (window as any).go.api.App.IngestEvidence(srcPath, outputDir, passphrase, vaultID);
  return typeof result === 'string' ? JSON.parse(result) : result;
}
...
export async function GetFavorites(): Promise<FavoriteEntry[]> {
  try {
    const result = await (window as any).go.api.App.GetFavorites();
    return typeof result === 'string' ? JSON.parse(result) : result || [];
  } catch {
    return [];
  }
...
export async function AddFavorite(command: string, label: string): Promise<void> {
  await (window as any).go.api.App.AddFavorite(command, label);
}

export async function RemoveFavorite(label: string): Promise<void> {
  await (window as any).go.api.App.RemoveFavorite(label);
}
...
export async function ExecuteCapability(capabilityID: string, params: any): Promise<string> {
  ...
  return await (window as any).go.api.App.ExecuteCapability(capabilityID, paramsStr);
}

export async function GetJobStatus(jobID: string): Promise<any> {
  const result = await (window as any).go.api.App.GetJobStatus(jobID);
  return typeof result === 'string' ? JSON.parse(result) : result;
}

export async function ListCapabilities(): Promise<string[]> {
  const result = await (window as any).go.api.App.ListCapabilities();
  return typeof result === 'string' ? JSON.parse(result) : result || [];
}
...
export async function HashFile(vfsPath: string, verifyHash?: string): Promise<HashResult> {
  const result = await (window as any).go.api.App.HashFile(vfsPath, verifyHash || '');
  return typeof result === 'string' ? JSON.parse(result) : result;
}
...
export async function BatchHashFiles(vfsPaths: string[]): Promise<BatchHashResult[]> {
  const result = await (window as any).go.api.App.BatchHashFiles(vfsPaths);
  return typeof result === 'string' ? JSON.parse(result) : result;
}
...
export async function PartitionHash(partitionIndex: number): Promise<PartitionHashResult> {
  const result = await (window as any).go.api.App.PartitionHash(partitionIndex);
  return typeof result === 'string' ? JSON.parse(result) : result;
}
...
export async function CompareHash(pathA: string, pathB: string): Promise<CompareResult> {
  const result = await (window as any).go.api.App.CompareHash(pathA, pathB);
  return typeof result === 'string' ? JSON.parse(result) : result;
}
...
export async function StartPartitionHashJob(partitionIndex: number): Promise<string> {
  return await (window as any).go.api.App.StartPartitionHashJob(partitionIndex);
}

export async function StartDirectoryHashJob(vfsPath: string): Promise<string> {
  return await (window as any).go.api.App.StartDirectoryHashJob(vfsPath);
}

export async function GetHashJobStatus(jobID: string): Promise<HashJobProgress> {
  const result = await (window as any).go.api.App.GetHashJobStatus(jobID);
  return typeof result === 'string' ? JSON.parse(result) : result;
}

export async function CancelHashJob(jobID: string): Promise<boolean> {
  return await (window as any).go.api.App.CancelHashJob(jobID);
}

export async function StartBatchHashJob(vfsPaths: string[]): Promise<string> {
  return await (window as any).go.api.App.StartBatchHashJob(vfsPaths);
}
...
export async function MountDisk(imagePath: string): Promise<any[]> {
  const result = await (window as any).go.api.StorageHandler.MountDisk(imagePath);
...
export async function MountPartition(index: number): Promise<boolean> {
  const result = await (window as any).go.api.StorageHandler.MountPartition(index);
  return !!result;
}

export async function StorageListDirectory(path: string): Promise<any[]> {
  const result = await (window as any).go.api.StorageHandler.ListDirectory(path);
  return Array.isArray(result) ? result : (typeof result === 'string' ? JSON.parse(result) : result || []);
}
